(function () {
  ...
  function resolveDoc(path) {
    ...
    const task = fetch('docs/' + encodeURIComponent(path))
      ...
      .then((text) => {
        const html = /\.html$/i.test(path) ? text : renderMarkdown(text);
  ...
  let searchIndex = null;        // search-index.json，懒加载后常驻
  let searchIndexTask = null;
  let searchSeq = 0;
  let pendingKeyword = '';       // 点搜索结果时记下关键词，文档打开后定位用
  let currentPath = null;        // 当前展示的文档，阅读位置记忆按它记账

  // 正文纯文本索引：构建期由 sync.py 生成，只取一次
  function loadSearchIndex() {
    if (searchIndex) return Promise.resolve(searchIndex);
    if (!searchIndexTask) {
      searchIndexTask = fetch('search-index.json', { cache: 'no-cache' })
        .then((r) => {
        ...
        .catch((err) => {
          searchIndexTask = null;   // 失败不缓存，下次可重试
          throw err;
        });
    }
  ...
  // 全文检索：标题/路径命中优先，其次按出现次数
  function fullTextHits(keyword) {
    ...
    for (const doc of searchIndex) {
      ...
      const occurrences = lowerText.split(lower).length - 1;
      const hitInTitle = doc.title.toLowerCase().includes(lower) || doc.path.toLowerCase().includes(lower);
      if (!occurrences && !hitInTitle) continue;

      const at = lowerText.indexOf(lower);
      hits.push({
        path: doc.path,
        title: doc.title,
        occurrences,
        hitInTitle,
        snippet: at === -1 ? '' : makeSnippet(text, at, keyword.length),
      });
    }
  ...
  function handleSearch(query) {
    ...
    renderSearchResults(searchIndex ? fullTextHits(keyword) : quickHits(keyword), keyword);
    ...
    loadSearchIndex()
      .then(() => {
        if (seq === searchSeq) renderSearchResults(fullTextHits(keyword), keyword);
      })
